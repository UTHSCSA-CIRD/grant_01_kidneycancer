...
#' 
agecuts <- c(0,10*2:7,Inf);
#' data dictionary:
#not needed yet
dctfile <- 'datadictionary.csv';
...
d1 <- mutate(d0,agebin=cut(d0$AGE,agecuts,include.lowest = T)) %>% 
  group_by(agebin) %>% 
  select(one_of(c('agebin',v(c_intnum)))) %>% 
  summarise_all(sum);

#' ## Create the combinations of rows for the output
rowslist <- list(
   Kidney_Both=v(!c_lc,!c_m,!c_f,!c_meta,c_cnt)
  ,Kidney_Male=v(!c_lc,c_m,!c_meta,c_cnt)
  ,Kidney_Female=v(!c_lc,c_f,!c_meta,c_cnt)
  ,Liver_Both=v(!c_kc,!c_m,!c_f,!c_meta,c_cnt)
  ,Liver_Male=v(!c_kc,c_m,!c_meta,c_cnt)
  ,Liver_Female=v(!c_kc,c_f,!c_meta,c_cnt)
);

#' ## Create the master output table
t00 <- sapply(names(rowslist),function(ii) {
  grps<-strsplit(ii,'_')[[1]];
  cbind(grps[1],grps[2]
        ,d1[,c('agebin',rowslist[[ii]])]) %>% 
    setNames(c('cancer','sex','agebin', 'N NHW','n NHW','n NHW TCR','N HSP','n HSP','n HSP TCR'))
  },simplify=F);

t01 <- do.call(rbind,t00);
write_tsv(t01,path=paste0(outputdir,'cancer_sex_age_hsp.csv'));
# 
